...
class SpecificationGenerator:
        ...
        try:
            ...
            for i, section_type in enumerate(section_types):
                try:
                    ...
                    print(f"Warning: Failed to generate {section_type.value} section: {e}")
            ...
            if include_tests:
                try:
                    test_scaffold = self.generate_test_scaffold(diagram_data, acceptance_criteria, test_language)
                except Exception as e:
                    # Log error but continue without tests
                    print(f"Warning: Failed to generate test scaffold: {e}")
            
            # Generate markdown content
            markdown_content = self._generate_markdown(title, sections, acceptance_criteria)
    ...
    def _validate_diagram_data(self, diagram_data: DiagramData) -> None:
        ...
        if not diagram_data:
            raise SpecificationGenerationError("Diagram data is None", "input_validation")
        
        if not diagram_data.nodes:
            raise SpecificationGenerationError("Diagram has no nodes", "input_validation")
        
        if not diagram_data.mermaid_syntax:
            raise SpecificationGenerationError("Diagram has no Mermaid syntax", "input_validation")
    ...
    def _generate_overview_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
        ...
        if entity_types:
            ...
            for entity_type, count in entity_types.items():
                ...
                content_parts.append(f"- **{type_description}**: {count} component{'s' if count != 1 else ''}")
    ...
    def _generate_requirements_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
    ...
    def _generate_architecture_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
        ...
        for entity_type, nodes in components_by_type.items():
            ...
            for node in nodes:
                content_parts.append(f"- **{node.label}**: {self._generate_component_description(node)}")
    ...
    def _generate_components_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
        ...
        for node in diagram_data.nodes:
            ...
            responsibilities = self._generate_component_responsibilities(node, diagram_data.edges)
            if responsibilities:
                content_parts.extend([
                    "**Responsibilities**:",
                    "",
    ...
    def _generate_data_flow_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
    ...
    def _generate_business_rules_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
        ...
        for edge in diagram_data.edges:
            ...
            if source_node and target_node:
                rule = self._generate_business_rule(source_node, target_node, edge, rule_counter)
                if rule:
    ...
    def _generate_acceptance_criteria_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
        ...
        criteria_counter = 1
        for node in diagram_data.nodes:
            criteria = self._generate_node_acceptance_criteria(node, diagram_data.edges, criteria_counter)
    ...
    def _generate_implementation_notes_section(self, diagram_data: DiagramData, order: int) -> SpecSection:
    ...
    def _generate_acceptance_criteria(self, diagram_data: DiagramData) -> List[AcceptanceCriterion]:
        ...
        for node in diagram_data.nodes:
            node_criteria = self._generate_node_criteria(node, diagram_data.edges, criterion_counter)
            criteria.extend(node_criteria)
            criterion_counter += len(node_criteria)
        
        # Generate criteria for relationships
        for edge in diagram_data.edges:
            edge_criteria = self._generate_edge_criteria(edge, diagram_data.nodes, criterion_counter)
            if edge_criteria:
    ...
    def _generate_markdown(self, title: str, sections: List[SpecSection], criteria: List[AcceptanceCriterion]) -> str:
        ...
        if not any(section.section_type == SpecificationSection.ACCEPTANCE_CRITERIA for section in sections):
            content_parts.extend([
            ...
            for criterion in high_priority_criteria[:5]:  # Show top 5
                content_parts.append(f"- **AC-{criterion.id}**: {criterion.description}")
    ...
    def _get_diagram_purpose(self, diagram_type: DiagramType) -> str:
        ...
        return purposes.get(diagram_type, "This diagram represents the system structure and relationships.")
    ...
    def _generate_functional_requirement(self, source: Node, target: Node, edge: Edge, req_num: int) -> str:
    ...
    def _generate_component_description(self, node: Node) -> str:
        ...
        return type_descriptions.get(node.entity_type, f"A system component for {node.label.lower()}")
    
    def _generate_component_responsibilities(self, node: Node, edges: List[Edge]) -> List[str]:
        ...
        for edge in outgoing:
            if edge.relationship_type == "creates":
                responsibilities.append(f"Create and manage {edge.label or 'related data'}")
        ...
        for edge in incoming:
            if edge.relationship_type == "uses":
                responsibilities.append(f"Provide services for {edge.label or 'system operations'}")
        
        return responsibilities[:3]  # Limit to top 3 responsibilities
    
    def _generate_component_interfaces(self, node: Node, edges: List[Edge]) -> List[str]:
        ...
        incoming = [edge for edge in edges if edge.target_id == node.id]
        if incoming:
            interfaces.append(f"Input: Receives data from {len(incoming)} source{'s' if len(incoming) != 1 else ''}")
        
        # Output interfaces
        outgoing = [edge for edge in edges if edge.source_id == node.id]
        if outgoing:
            interfaces.append(f"Output: Sends data to {len(outgoing)} target{'s' if len(outgoing) != 1 else ''}")
    ...
    def _analyze_interaction_patterns(self, diagram_data: DiagramData) -> List[str]:
        ...
        hub_nodes = [node_id for node_id, count in connection_counts.items() if count > 3]
        if hub_nodes:
            patterns.append(f"Central hub pattern with {len(hub_nodes)} highly connected component{'s' if len(hub_nodes) != 1 else ''}")
            ...
            if node.id not in visited:
                ...
                if len(flow) > 1:
                    flows.append({
                        ...
                        "steps": [f"{flow[i].label} → {flow[i+1].label}" for i in range(len(flow)-1)]
                    })
        
        return flows[:3]  # Limit to top 3 flows
    
    def _trace_flow_from_node(self, start_node: Node, edges: List[Edge], visited: set) -> List[Node]:
        ...
        if outgoing:
            ...
            next_node = self._find_node_by_id([n for n in flow if n.id != start_node.id], next_edge.target_id)
            if next_node and next_node.id not in visited:
                flow.extend(self._trace_flow_from_node(next_node, edges, visited))
        
        return flow
    
    def _generate_business_rule(self, source: Node, target: Node, edge: Edge, rule_num: int) -> Optional[str]:
    ...
    def _generate_node_acceptance_criteria(self, node: Node, edges: List[Edge], start_num: int) -> List[str]:
        """Generate acceptance criteria for a node."""
        criteria = []
        
        # Basic existence criteria
        criteria.append(f"**AC-{start_num}**: {node.label} component must be properly initialized")
        
        # Functional criteria based on relationships
        outgoing = [edge for edge in edges if edge.source_id == node.id]
        if outgoing:
            criteria.append(f"**AC-{start_num + 1}**: {node.label} must successfully interact with {len(outgoing)} connected component{'s' if len(outgoing) != 1 else ''}")
        
        return criteria[:2]  # Limit to 2 criteria per node
    
    def _generate_node_criteria(self, node: Node, edges: List[Edge], start_num: int) -> List[AcceptanceCriterion]:
        ...
        connected_edges = [edge for edge in edges if edge.source_id == node.id or edge.target_id == node.id]
        if connected_edges:
            criteria.append(AcceptanceCriterion(
                id=str(start_num + 1),
    ...
    def _generate_edge_criteria(self, edge: Edge, nodes: List[Node], criterion_num: int) -> Optional[AcceptanceCriterion]:
    ...
    def generate_test_scaffold(
        ...
        try:
            ...
            # Generate unit tests
            unit_test_file = self._generate_unit_test_file(diagram_data, acceptance_criteria, language)
            test_files.append(unit_test_file)
            
            # Generate integration tests
            integration_test_file = self._generate_integration_test_file(diagram_data, acceptance_criteria, language)
            ...
            if len(diagram_data.edges) > 2:  # Complex enough for E2E tests
                e2e_test_file = self._generate_e2e_test_file(diagram_data, acceptance_criteria, language)
    ...
    def _generate_unit_test_file(
        ...
        for node in diagram_data.nodes:
            node_test_cases = self._generate_node_unit_tests(node, acceptance_criteria, language)
    ...
    def _generate_integration_test_file(
        ...
        for edge in diagram_data.edges:
            edge_test_cases = self._generate_edge_integration_tests(edge, diagram_data.nodes, acceptance_criteria, language)
        ...
        # Generate full file content
        full_content = self._generate_test_file_content(
            "integration_tests", imports, setup_code, test_cases, helper_methods, language
        )
    ...
    def _generate_e2e_test_file(
        ...
        for workflow in workflows:
            workflow_test_cases = self._generate_workflow_e2e_tests(workflow, acceptance_criteria, language)
    ...
    def _generate_node_unit_tests(
        ...
        related_criteria = [
            criterion for criterion in acceptance_criteria
        ...
        for criterion in related_criteria[:2]:  # Limit to 2 criteria per node
            criterion_test = self._generate_acceptance_criteria_test(node, criterion, language)
    ...
    def _generate_edge_integration_tests(
        ...
        success_test = self._generate_successful_interaction_test(source_node, target_node, edge, language)
        test_cases.append(success_test)
        
        # Generate failure handling test
        failure_test = self._generate_interaction_failure_test(source_node, target_node, edge, language)
    ...
    def _generate_basic_functionality_test(self, node: Node, language: ProgrammingLanguage) -> TestCase:
        ...
        template = test_templates.get(language, test_templates[ProgrammingLanguage.PYTHON])
    ...
    def _generate_validation_test(self, node: Node, language: ProgrammingLanguage) -> TestCase:
    ...
    def _generate_error_handling_test(self, node: Node, language: ProgrammingLanguage) -> TestCase:
    ...
    def _generate_successful_interaction_test(
        ...
        return TestCase(
            ...
            test_code=template["test"],
            teardown_code=template["teardown"],
            assertions=[f"Verify {source.label} can {edge.relationship_type} {target.label}"],
            related_acceptance_criteria=[]
        )
    ...
    def _generate_interaction_failure_test(
        ...
        return TestCase(
            ...
            test_code=template["test"],
            teardown_code=template["teardown"],
            assertions=[f"Verify {source.label} handles {edge.relationship_type} failure gracefully"],
            related_acceptance_criteria=[]
        )
    
    def _generate_happy_path_test(self, workflow: List[Node], language: ProgrammingLanguage) -> TestCase:
        """Generate happy path E2E test."""
        workflow_name = "_to_".join([node.label.lower().replace(' ', '_') for node in workflow[:3]])
    ...
    def _generate_error_path_test(self, workflow: List[Node], language: ProgrammingLanguage) -> TestCase:
    ...
    def _format_test_case(self, test_case: TestCase, language: ProgrammingLanguage) -> List[str]:
...
"""
    ...
    def _identify_workflows(self, diagram_data: DiagramData) -> List[List[Node]]:
        ...
        for node in diagram_data.nodes:
            if node.id not in visited:
                workflow = self._trace_workflow_from_node(node, diagram_data.edges, visited)
                if len(workflow) > 1:
                    workflows.append(workflow)
        
        return workflows[:3]  # Limit to top 3 workflows
    
    def _trace_workflow_from_node(self, start_node: Node, edges: List[Edge], visited: set) -> List[Node]:
        ...
        outgoing = [edge for edge in edges if edge.source_id == start_node.id]
        for edge in outgoing:
            target_node = self._find_node_by_id([n for n in workflow if n.id != start_node.id], edge.target_id)
            if target_node and target_node.id not in visited:
                workflow.extend(self._trace_workflow_from_node(target_node, edges, visited))
