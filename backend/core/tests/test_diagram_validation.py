...
from core.services.diagram_engine import (DiagramEngine,
...
class TestDiagramValidation:
        ...
        for i in range(len(complex_entities) - 1):
            for j in range(i + 1, min(i + 4, len(complex_entities))):
                complex_relationships.append(
                    Relationship(
                        f"Entity{i}", f"Entity{j}", "uses", "", 0.9
                    )
                )
    ...
    def test_complexity_score_calculation(self):
        ...
        complex_entities = [Entity(f"E{i}", EntityType.OBJECT, {}, 0.9, 0, 1) for i in range(30)]
        complex_relationships = [
            Relationship(f"E{i}", f"E{j}", "uses", "", 0.9)
            for i in range(29) for j in range(i+1, min(i+5, 30))
        ]
    ...
    def test_validation_metadata(self):
        ...
        expected_metadata_keys = [
            "node_count", "edge_count", "syntax_length", 
            "has_positions", "error_count", "warning_count", "complexity_score"
        ]
